...
// mapalloc.go (untagged) references them for type-checking; they never execute
// here because compositeMapType returns false, so PlanMapSlots returns the lazy
// plan (MapAllocUnit only) and InitMapSlots writes dirPtr=nil. The struct shape
// mirrors the swissmap definition so layout.GroupType / GroupSize field accesses
// compile.
...
func ReadMapLayout(_ unsafe.Pointer) SwissMapLayout {
