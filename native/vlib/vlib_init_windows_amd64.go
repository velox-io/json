//go:build windows && amd64 && !vj_nolookup
...
func init() {
	...
	lookupInit = lookupInitReserved
	getTier = vjLookupGetTier
	footprint = vjLookupFootprint
}

func lookupInitReserved(storage unsafe.Pointer, storageSize uintptr, cfg *Config) int32 {
	return vjLookupInit(storage, storageSize, cfg)
}
