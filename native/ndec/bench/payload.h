/*
 * Shared JSON loader for all ndec benchmarks.
 *
 * Loads a JSON file into a heap buffer at startup.
 *
...
/* Tail padding past the JSON payload. Sized for ndec's atof single-shot
 * SWAR load (8 bytes). Other backends (yyjson, simdjson, sonic) do their
 * own padding internally and tolerate the extra bytes. */
#define BENCH_PAYLOAD_PADDED_TAIL 8
...
static char *bench_payload_read_file(const char *path, size_t *out_len) {
  ...
  char *buf = (char *)malloc((size_t)sz + BENCH_PAYLOAD_PADDED_TAIL);
  if (!buf) {
    fclose(fp);
    return NULL;
  }
  ...
  /* Zero the tail so any over-read past the last number token sees a
   * non-digit byte (atof's padded entry stops naturally on the first
   * non-digit). */
  memset(buf + sz, 0, BENCH_PAYLOAD_PADDED_TAIL);
  if (out_len)
    *out_len = (size_t)sz;
  return buf;
}

/* Resolve the payload path from argv / env. */
static const char *bench_payload_resolve_path(int argc, char **argv) {
  if (argc >= 2 && argv[1] && argv[1][0] != '\0')
    return argv[1];
  return getenv("NDEC_BENCH_PAYLOAD");
}
...
static char *bench_payload_load(int argc, char **argv, size_t *out_len) {
  ...
  if (!path || !path[0]) {
    fprintf(stderr, "bench_payload: no payload specified; use argv[1] or $NDEC_BENCH_PAYLOAD\n");
    exit(1);
  }
...
static void bench_payload_free(char *buf) {
  free(buf);
}
