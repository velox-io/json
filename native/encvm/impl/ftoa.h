...
/* POW10TAB lives in a separate generated header.  Kept as the sole includer
 * so the (static const) table has exactly one TU-local copy.
 * Must come after the us_pm_hilo typedef: the table is typed as us_pm_hilo. */
...
INLINE uint64_t us_floor(us_unrounded u) {
  return (u + 0) >> 2;
}

/* Round half-to-even */
INLINE uint64_t us_round(us_unrounded u) {
  return (u + 1 + ((u >> 2) & 1)) >> 2;
}

INLINE uint64_t us_ceil(us_unrounded u) {
  return (u + 3) >> 2;
}

INLINE us_unrounded us_nudge(us_unrounded u, int delta) {
...
/* floor(x * log10(2)) */
INLINE int us_log10Pow2(int x) {
  return (x * 78913) >> 18;
}

/* floor(x * log2(10)) */
INLINE int us_log2Pow10(int x) {
  return (x * 108853) >> 15;
}

/* floor(log10(3/4 * 2^e)) = floor(e*log10(2) - log10(4/3)) */
INLINE int us_skewed(int e) {
  return (e * 631305 - 261663) >> 21;
}

/* Core uscale */

INLINE us_scaler us_prescale(int e, int p, int lp) {
  us_scaler c;
  c.pm = POW10TAB[p + 348];
  c.s  = -(e + lp + 3);
  return c;
 ...
 */
INLINE us_unrounded us_uscale(uint64_t x, us_scaler c) {
...
INLINE uint64_t us_rotr64(uint64_t x, int k) {
  return (x >> k) | (x << (64 - k));
}

/* Remove trailing decimal zeros from x * 10^p; updates *xp / *pp in place. */
INLINE void us_trim_zeros(uint64_t *xp, int *pp) {
  uint64_t x = *xp;
  int p      = *pp;
...
INLINE void us_unpack64(double f, uint64_t *m, int *e) {
  uint64_t bits;
 ...
 */
INLINE void us_short64(double f, uint64_t *d_out, int *p_out) {
...
// clang-format off
INLINE uint32_t us_decimal_length17(const uint64_t v) {
...
INLINE int us_write_mantissa_digits(uint8_t *buf, uint64_t mantissa, uint32_t olength) {
  uint32_t i = olength;
  if (mantissa >> 32 != 0) {
    ...
    i -= 2;
    __builtin_memcpy(buf + i, DIGIT_PAIRS + c0, 2);
    i -= 2;
    __builtin_memcpy(buf + i, DIGIT_PAIRS + c1, 2);
    i -= 2;
    __builtin_memcpy(buf + i, DIGIT_PAIRS + d0, 2);
    i -= 2;
    __builtin_memcpy(buf + i, DIGIT_PAIRS + d1, 2);
  }
  ...
  if (output2 >= 100) {
    ...
    output2 /= 100;
    i -= 2;
    __builtin_memcpy(buf + i, DIGIT_PAIRS + c, 2);
  }
  if (output2 >= 10) {
...
INLINE int vj_write_float64(uint8_t *buf, double value, int flags) {
  uint64_t bits;
...
/* NOINLINE: float32 fields are rare, keeps the body out of the VM function. */
NOINLINE static int vj_write_float32(uint8_t *buf, float value, int flags) {
  uint32_t bits;
   ...
   * Subnormal: significant bits = bit length of ieeeMantissa -> z = 64 - sigbits */
  int sigbits;
  if (ieeeExponent != 0) {
  ...
  if (m == ((uint64_t)1 << 63) && ieeeExponent > 0 && ieeeMantissa == 0 && ieeeExponent > 1) {
    p2 = -us_skewed(e + z);
    mn = m - ((uint64_t)1 << (z - 2));
  } else {
    if (e < minExp) {
