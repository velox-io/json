...
static inline uint8_t *vj_seq_write_indent(uint8_t *buf,
...
static inline uint8_t *vj_seq_encode_float64(uint8_t *buf,
...
static inline uint8_t *vj_seq_encode_int64(uint8_t *buf,
                                           const uint8_t *elem_ptr,
                                           uint32_t flags, int *nan_inf) {
  (void)flags;
  (void)nan_inf;
...
static inline uint8_t *vj_seq_encode_string(uint8_t *buf,
                                            const uint8_t *elem_ptr,
                                            uint32_t flags, int *nan_inf) {
  (void)nan_inf;
...
static inline int64_t vj_seq_need_int64(const uint8_t *elem_ptr, int ipad) {
  (void)elem_ptr;
  return 1 + ipad + 21;
}

static inline int64_t vj_seq_need_string(const uint8_t *elem_ptr, int ipad) {
  const GoString *s = (const GoString *)elem_ptr;
...
INLINE VjSeqResult vj_seq_iterate_impl(
    uint8_t *buf, const uint8_t *bend, const uint8_t *data, int64_t count,
...
NOINLINE static VjSeqResult
vj_seq_iterate_float64(uint8_t *buf, const uint8_t *bend, const uint8_t *data,
                       int64_t count, int32_t start_idx, uint32_t flags,
                       const VjSeqIndent *ind) {
  return vj_seq_iterate_impl(buf, bend, data, count, start_idx, flags, ind, 8,
                             1, vj_seq_encode_float64, vj_seq_need_float64);
}

NOINLINE static VjSeqResult
vj_seq_iterate_int(uint8_t *buf, const uint8_t *bend, const uint8_t *data,
                   int64_t count, int32_t start_idx, uint32_t flags,
                   const VjSeqIndent *ind) {
  return vj_seq_iterate_impl(buf, bend, data, count, start_idx, flags, ind, 8,
                             0, vj_seq_encode_int64, vj_seq_need_int64);
}

NOINLINE static VjSeqResult
vj_seq_iterate_int64(uint8_t *buf, const uint8_t *bend, const uint8_t *data,
                     int64_t count, int32_t start_idx, uint32_t flags,
                     const VjSeqIndent *ind) {
  return vj_seq_iterate_impl(buf, bend, data, count, start_idx, flags, ind, 8,
                             0, vj_seq_encode_int64, vj_seq_need_int64);
...
NOINLINE static VjSeqResult
vj_seq_iterate_string(uint8_t *buf, const uint8_t *bend, const uint8_t *data,
                      int64_t count, int32_t start_idx, uint32_t flags,
                      const VjSeqIndent *ind) {
  return vj_seq_iterate_impl(buf, bend, data, count, start_idx, flags, ind, 16,
                             0, vj_seq_encode_string, vj_seq_need_string);
}
...
#define VJ_DEFINE_SEQ_OP(OP_LABEL, TRACE_LABEL, ITERATE_FN, HAS_NAN_CHECK)     \
  OP_LABEL: {                                                                  \
    ...
    {                                                                          \
      if (__builtin_expect(                                                    \
              VJ_ST_GET_STACK_DEPTH(vmstate) >= VJ_MAX_STACK_DEPTH, 0)) {      \
      ...
      VjSeqIndent ind = {                                                      \
          ...
          (uint8_t)(indent_prefix_len),                                        \
                                                                               ...
                                                                               \
      VjSeqResult r =                                                          \
          ITERATE_FN(buf, bend, seq_data, seq_count, seq_start_idx,            \
