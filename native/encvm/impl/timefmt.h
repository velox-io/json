...
static inline int vj_write_rfc3339nano(uint8_t *buf, const GoTime *t,
  ...
  *buf++ = '"';
  {
    int y = year;
    buf[0] = (uint8_t)('0' + y / 1000); y %= 1000;
    buf[1] = (uint8_t)('0' + y / 100);  y %= 100;
    buf[2] = (uint8_t)('0' + y / 10);
    buf[3] = (uint8_t)('0' + y % 10);
    buf += 4;
  }
  *buf++ = '-';
  vj_write_2d(buf, (int)month); buf += 2;
  *buf++ = '-';
  vj_write_2d(buf, (int)mday); buf += 2;
  *buf++ = 'T';
  vj_write_2d(buf, (int)hour); buf += 2;
  *buf++ = ':';
  vj_write_2d(buf, (int)min); buf += 2;
  *buf++ = ':';
  vj_write_2d(buf, (int)sec); buf += 2;
  ...
  if (tz_offset == 0) {
    *buf++ = 'Z';
    ...
    int off = tz_offset;
    if (off < 0) { *buf++ = '-'; off = -off; }
    else         { *buf++ = '+'; }
    vj_write_2d(buf, off / VJ_SECONDS_PER_HOUR); buf += 2;
    *buf++ = ':';
    vj_write_2d(buf, (off % VJ_SECONDS_PER_HOUR) / VJ_SECONDS_PER_MINUTE); buf += 2;
  }
