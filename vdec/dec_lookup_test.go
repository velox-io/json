...
func lookupField(dec *DecStructInfo, key string) *DecFieldInfo {
	return dec.LookupFieldBytes([]byte(key))
}

// makeTestStructInfo builds a DecStructInfo from a list of field names.
// This bypasses reflect to create controlled test scenarios.
func makeTestStructInfo(names []string) *DecStructInfo {
	dec := &DecStructInfo{
		Fields: make([]DecFieldInfo, len(names)),
	}
	for i, name := range names {
		dec.Fields[i] = DecFieldInfo{
			JSONName: name,
			Offset:   uintptr(i * 8),
			Kind:     typ.KindString,
		}
...
func TestBuildLookup_SingleField(t *testing.T) {
	dec := makeTestStructInfo([]string{"id"})
...
func TestLookup_CaseInsensitive(t *testing.T) {
	dec := makeTestStructInfo([]string{"Name", "Age", "Email"})
...
func TestLookup_UnknownKeys(t *testing.T) {
	dec := makeTestStructInfo([]string{"id", "name", "email", "phone", "address"})
...
func TestLookup_SingleCharFields(t *testing.T) {
	dec := makeTestStructInfo([]string{"a", "b", "c", "d", "e", "f"})
...
func TestLookup_UnicodeFieldNames(t *testing.T) {
	dec := makeTestStructInfo([]string{"名前", "年齢", "メール", "住所", "電話"})
...
func TestLookup_ViaReflect(t *testing.T) {
	...
	dec := DecTypeInfoOf(reflect.TypeOf(User{})).ResolveStruct()
...
func TestLookup_ViaReflect_LargeStruct(t *testing.T) {
	...
	dec := DecTypeInfoOf(reflect.TypeOf(BigStruct{})).ResolveStruct()
...
func BenchmarkLookup_Linear_4fields(b *testing.B) {
	dec := makeTestStructInfo([]string{"id", "name", "email", "age"})
	key := []byte("email")
	b.ResetTimer()
	for range b.N {
...
func BenchmarkLookup_Bitmap_8fields(b *testing.B) {
	dec := makeTestStructInfo([]string{"id", "name", "email", "phone", "address", "city", "state", "zip"})
	key := []byte("address")
	b.ResetTimer()
	for range b.N {
		dec.LookupFieldBytes(key)
...
func BenchmarkLookup_RawMap_8fields(b *testing.B) {
	names := []string{"id", "name", "email", "phone", "address", "city", "state", "zip"}
	m := make(map[string]*DecFieldInfo, len(names))
	fields := make([]DecFieldInfo, len(names))
	for i, name := range names {
		fields[i] = DecFieldInfo{JSONName: name}
		m[name] = &fields[i]
	}
	key := "address"
	b.ResetTimer()
...
func BenchmarkLookup_RawMap_16fields(b *testing.B) {
	names := make([]string, 16)
	fields := make([]DecFieldInfo, 16)
	m := make(map[string]*DecFieldInfo, 16)
	for i := range 16 {
		names[i] = fmt.Sprintf("field_%d", i)
		fields[i] = DecFieldInfo{JSONName: names[i]}
		m[names[i]] = &fields[i]
	}
	key := "field_12"
	b.ResetTimer()
