...
func TestProcessEscapedStringKinds(t *testing.T) {
	...
	t.Run("typ.KindString", func(t *testing.T) {
		ti := &DecTypeInfo{Kind: typ.KindString}
		var s string
	...
	t.Run("typ.KindAny", func(t *testing.T) {
		sc.arenaOff = 0
		ti := &DecTypeInfo{Kind: typ.KindAny}
		var a any
	...
	t.Run("typ.KindInt returns error", func(t *testing.T) {
		sc.arenaOff = 0
		ti := &DecTypeInfo{Kind: typ.KindInt, Type: reflect.TypeOf(0)}
		var dummy int
		_, err := sc.processEscapedString(src, 0, 5, ti, unsafe.Pointer(&dummy))
		if err == nil {
			t.Fatal("expected error for typ.KindInt")
		}
