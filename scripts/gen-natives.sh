...
# Build provenance stamp. Every syso records the commit, the content hash
# of the native input set, and whether the tree was dirty, so the lineage
# guard in pgo-download-syso.sh can verify an artifact against the
# installing tree from the artifact alone, with no external metadata.
source "$REPO_ROOT/scripts/native-build-inputs.sh"
BUILD_COMMIT=$(git rev-parse HEAD 2>/dev/null || echo unknown)
BUILD_INPUTS=$(native_inputs_hash)
if native_inputs_dirty; then
  BUILD_DIRTY=1
else
  BUILD_DIRTY=0
fi
_module=$(basename "$TARGET_DIR")
_stamp_c="$OUTPUT_DIR/build_info_${_module}.c"
cat >"$_stamp_c" <<EOF
/* Generated by gen-natives.sh; do not edit. Provenance read by
 * pgo-download-syso.sh to verify artifact lineage. */
const char ${_module}_build_info[] =
    "commit=$BUILD_COMMIT\n"
    "inputs=$BUILD_INPUTS\n"
    "dirty=$BUILD_DIRTY\n";
EOF
EXTRA_SOURCES="$EXTRA_SOURCES $_stamp_c"
