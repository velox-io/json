...
# Runs the plain leg, then the same leg under -race when available.
function Invoke-GoTestRace {
    param([string[]]$Args)
    Invoke-GoTest @Args
    if ($RaceArgs.Count -gt 0) {
        Invoke-GoTest @RaceArgs @Args
    }
}

Invoke-GoTestRace . -count=1

Invoke-GoTestRace ./vbind ./decode/bind/ -count=1

Invoke-GoTestRace ./venc -count=1
Invoke-GoTestRace -tags vj_noencvm ./venc -count=1
Invoke-GoTestRace -tags vjgcstress ./venc -count=1
Invoke-GoTestRace -tags vjstress ./venc -count=1

Invoke-GoTestRace ./tests/ -count=1
Invoke-GoTestRace -tags vj_noencvm ./tests/ -count=1
Invoke-GoTestRace -tags vjgcstress ./tests/ -count=1
...
Invoke-GoTestRace -tags vjgcstress ./tests/preemptstress -count=1

Invoke-GoTestRace ./tests/compat/ -count=1

# Invoke-GoTestRace ./ndec/... -count=1
...
if ($LASTEXITCODE -eq 0) {
    Invoke-GoTestRace -C ./benchmark . -count=1
    Invoke-GoTestRace -C ./benchmark -tags vj_noencvm . -count=1
