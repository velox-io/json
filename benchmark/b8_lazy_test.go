...
type encodingJSONCBGravatar struct {
	Avatars []*encodingJSONCBAvatar `json:"avatars"`
}
...
type encodingJSONCBPerson struct {
	Name     *encodingJSONCBName     `json:"name"`
	Github   *encodingJSONCBGithub   `json:"github"`
	Gravatar *encodingJSONCBGravatar `json:"gravatar"`
}

type encodingJSONMediumPayload struct {
	Person  *encodingJSONCBPerson  `json:"person"`
...
func Benchmark_Project_Medium_Velox(b *testing.B) {
	for i := 0; i < b.N; i++ {
		var data encodingJSONMediumPayload
		vjson.Unmarshal(mediumFixture, &data)
...
type encodingJSONDSTopicsList struct {
	Topics        []*encodingJSONDSTopic `json:"topics"`
	MoreTopicsUrl string                 `json:"more_topics_url"`
}

type encodingJSONLargePayload struct {
	Users  []*encodingJSONDSUser     `json:"users"`
	Topics *encodingJSONDSTopicsList `json:"topics"`
}
...
func Benchmark_Project_Large_Velox(b *testing.B) {
	for i := 0; i < b.N; i++ {
		var data encodingJSONLargePayload
		vjson.Unmarshal(largeFixture, &data)
