...
func TestUnmarshal_CircularMutual(t *testing.T) {
	...
	var vj circA
	if err := vjson.Unmarshal([]byte(input), &vj); err != nil {
...
func TestEmbed_MixedFieldTypes(t *testing.T) {
	...
	if err := vjson.Unmarshal(got, &vjVal); err != nil {
		t.Fatalf("Unmarshal: %v", err)
	}
...
// PartialTypeInfoOf returns a partial TypeInfo with nil Codec, which is
// value-copied into StructCodec.Fields[]. ResolveCodec() must lazily
// resolve it from the TypeInfoCache at usage time.
//
...
func TestRoundtrip_IndirectCycle(t *testing.T) {
	...
	err = vjson.Unmarshal(data, &got)
	if err != nil {
		t.Fatalf("Unmarshal error: %v", err)
	}
	data2, err := vjson.Marshal(got)
	if err != nil {
