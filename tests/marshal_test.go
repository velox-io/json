...
func TestAppendMarshal(t *testing.T) {
	...
	prefix := []byte("PREFIX:")
	got, err := vjson.AppendMarshal(prefix, v)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAppendMarshal_BufferRetainedInPool(t *testing.T) {
	...
	v1 := small{X: 1}
	result1, err := vjson.AppendMarshal(dst, v1)
	if err != nil {
		t.Fatal(err)
	}
...
func TestMarshal_NilPointerValue(t *testing.T) {
	// vjson.Marshal((*T)(nil)) should produce "null"
	got, err := vjson.Marshal((*MarshalSmall)(nil))
	if err != nil {
		t.Fatal(err)
	}
	if string(got) != "null" {
