...
func TestNumber_WithoutUseNumber_Unmarshal(t *testing.T) {
	...
	input := []byte(`{"n":42}`)
	var result map[string]any
	if err := vjson.Unmarshal(input, &result); err != nil {
		t.Fatal(err)
	}
	v := result["n"]
...
func TestNumber_TopLevel(t *testing.T) {
	input := []byte(`42`)
	var n json.Number
	if err := vjson.Unmarshal(input, &n); err != nil {
		t.Fatal(err)
	}
	if string(n) != "42" {
...
// Float64 overflow: numbers that overflow float64 must be rejected (matching encoding/json)
...
func TestNumber_Float64Overflow(t *testing.T) {
	tests := []struct {
		...
		{"1e401", `1e401`},
		{"-1e999", `-1e999`},
		{"1e308_overflow", `1.8e308`},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Target: any (interface{})
			var a any
			vjErr := vjson.Unmarshal([]byte(tt.input), &a)
			var stdA any
			stdErr := json.Unmarshal([]byte(tt.input), &stdA)
			if stdErr != nil && vjErr == nil {
				t.Errorf("vjson accepted overflow %q into any but encoding/json rejected it", tt.input)
			}

			// Target: float64
			var f float64
			vjErr = vjson.Unmarshal([]byte(tt.input), &f)
			var stdF float64
			stdErr = json.Unmarshal([]byte(tt.input), &stdF)
			if stdErr != nil && vjErr == nil {
...
func TestNumber_LargeIntegerPrecision(t *testing.T) {
	tests := []struct {
		...
		{"24_digit_integer", `100000000000000000000000`},
		{"20_digit_integer", `10000000000000000000`},
		{"21_digit_integer", `123456789012345678901`},
		{"25_zeros", `1000000000000000000000000`},
		{"negative_24_digit", `-100000000000000000000000`},
		{"20_digit_frac", `0.10000000000000000001`},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var vjResult any
			if err := vjson.Unmarshal([]byte(tt.input), &vjResult); err != nil {
				t.Fatalf("vjson error: %v", err)
			}
			var stdResult any
			if err := json.Unmarshal([]byte(tt.input), &stdResult); err != nil {
